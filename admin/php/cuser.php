...
<div class="wrapper">
<div class="form">
<form method="POST" action="" name="user"  enctype="multipart/form-data"  style="max-width:500px;margin:auto">
  <h2 class="form-heading"><strong>Register New Customer</strong></h2>
  <?php
	if($_POST['btnsubmit'])
	{
		$fname=$_POST['fname'];
		...
		{
			echo "<p style=\"color:red\">Please Enter your Birth Date</p>";		
		}
    else if(empty($gen))
		{
			echo "<p style=\"color:red\">Please Enter your Gender</p>";		
		}
    else if(empty($phone))
		{
  ...
  <div class="input-container">
    ...
    <input  name="address" id="address" class="input-field" type="textarea" placeholder="Address" value="<?php if(isset($_POST['address'])) echo $_POST['address']; ?>"/>
  </div>
  <div class="input-container">
    <i class="fa fa-pinterest-p icon"></i>
    <input  name="pincode" id="pincode" class="input-field" type="text" placeholder="Pincode"  onkeydown = "return isNumeric(event.keyCode);" onpaste = "return false; " value="<?php if(isset($_POST['pincode'])) echo $_POST['pincode']; ?>"/>
  </div>
      <div class="input-container">
    <i class="fa fa-envelope icon"></i>
    <input id="email" name="email" class="input-field" type="email" placeholder="Email" value="<?php if(isset($_POST['email'])) echo $_POST['email']; ?>"/>
  </div>
   <div class="input-container">
    <i class="fa fa-birthday-cake icon"></i>
    <input  name="bdate" id="bdate" class="input-field" type="text" placeholder="Birth Date" onfocus="(this.type='date')" onblur="(this.type='text')"value="<?php if(isset($_POST['bdate'])) echo $_POST['bdate']; ?>" />
  </div>
  <div class="input-container">
  ...
  <input type="submit" class="btn" id="btnsubmit" name="btnsubmit" onclick="return User();" value="Create User" />
</form>
</div>
