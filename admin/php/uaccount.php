...
<form method="POST" action="" name="admin"  enctype="multipart/form-data" style="max-width:500px;margin:auto">
  <h2 class="form-heading"><strong>Update Your Account</strong></h2>
  <?php
  session_start();
  $id=$_SESSION['admin'];
...
$path=1;
		...
		{
			echo "<p style=\"color:red\">Please Enter Phone</p>";		
		}
		else
		{   
			...
			$sql="UPDATE `admin` SET `FirstName` = '$fname' , `LastName` = '$lname' ,  `Email` = '$email'  , `PhoneNO` = '$phone' WHERE `Adminid` = '$id' ;";
  ...
  <div class="input-container">
   ...
   <input type="submit" class="btn" id="btnsubmit" name="btnsubmit" onclick="return Admin();" value="Update Account" />
</form>
</div>
<br><br><br><br><br><br>
